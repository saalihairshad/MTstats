...
rt_tb 
chisq_rt <- chisq.test(rt_tb, simulate.p.value = TRUE)
chisq_rt
```
...
```{r}
p <- ggplot(fv_df, aes(x = country, y = count)) +
  geom_col(aes(color = favorites, fill = favorites), position = position_dodge(0.8), width = 0.7, size = 0.3, alpha = 0.4) +
  geom_text(
  aes(label = count, group = favorites), 
  position = position_dodge(0.8),
  vjust = -0.3, size = 3
) +
  theme_minimal()+
  labs(x="Country", y="Favorite Count", title = "Favorite Count by Country") +
  theme(legend.position = "right", axis.text.x = element_text(angle = 90, hjust = 0.5)) 
p
```
...
Only these 3 pairs differ
  <li>India : Netherlands</li>
  <li>Netherlands : United Kingdom</i>
  <li>France : India</li>
</span>
...
```{r}
#typeof(xx)
write.xlsx2(reply_posthoc, 'reply_1.xlsx', sheetName = "Sheet1",
